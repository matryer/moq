...
func TestBuildTag(t *testing.T) {
	m, err := New(Config{SrcDir: "testpackages/example", BuildTag: "example_build_tag"})
	if err != nil {
		t.Fatalf("moq.New: %s", err)
	}
	var buf bytes.Buffer
	...
	// check for exact match, since we need an empty line after the build tag
	header := strings.Join([]string{
		"// Code generated by moq; DO NOT EDIT.",
		"// github.com/matryer/moq",
		"",
		"//go:build example_build_tag",
		"",
		"package example",
		"",
		"import (",
	...
	if !strings.HasPrefix(s, header) {
		t.Log(s[:len(header)])
		t.Errorf("header mismatch: \"%s\"", header)
	}
}
