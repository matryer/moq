...
var moqTemplateNoPanic = `// Code generated by moq; DO NOT EDIT
// github.com/matryer/moq
...
{{ range .Methods }}
// {{.Name}} calls {{.Name}}Func.
func (mock *{{$obj.InterfaceName}}Mock) {{.Name}}({{.Arglist}}) {{.NamedReturnArglist}} {
	if mock.{{.Name}}Func != nil {
	callInfo := struct {
		{{- range .Params }}
		{{ .Name | Exported }} {{ .Type }}
		...
		lock{{$obj.InterfaceName}}Mock{{.Name}}.Unlock()
	{{- if .ReturnArglist }}
		{{.NamedReturnArgCallList}} = mock.{{.Name}}Func({{.ArgCallList}})
	{{- else }}
		mock.{{.Name}}Func({{.ArgCallList}})
	{{- end }}
	}
